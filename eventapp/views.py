...
class EntertainmentEventServiceItemAPIView(APIView):
    ...
    def get(self, request):
        ...
        if item_id:
            item = EntertainmentEventServiceItem.objects.filter(id=item_id).first()
            if not item:
                return Response(
                    {"success": False, "message": "Item not found"},
                    status=404
...
class PrivatePartiesEventServiceItemAPIView(APIView):
    ...
    def get(self, request):
        ...
        if item_id:
            item = PrivatePartiesEventServiceItem.objects.filter(id=item_id).first()
            if not item:
                return Response(
                    {"success": False, "message": "Item not found"},
                    status=404
...
class SeminarEventServiceItemAPIView(APIView):
    ...
    def get(self, request):
        ...
        if item_id:
            item = SeminarEventServiceItem.objects.filter(id=item_id).first()
            if not item:
                return Response(
                    {"success": False, "message": "Item not found"},
                    status=404
